  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = length(aws_ecr_repository.register_service_repo) > 0 ? aws_ecr_repository.register_service_repo[0].arn : aws_ecr_repository.register_service_repo[0].arn
      },
      {
        Effect = "Allow",
        Action = [
